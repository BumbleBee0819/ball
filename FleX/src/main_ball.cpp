...
void PrintSimParams() {  //---- Simulation Params
    printf("\n ---- Simulation Params ----\n");
    printf("substeps:                  %d\n", g_numSubsteps);
    printf("radius:                    %f\n", g_params.radius);
    printf("num_planes:                %d\n", g_params.numPlanes);
...
void ParseConfig(const char* path, int& n_iters, Ball::ClothParams& cp, Ball::ObjParams& op) {
    ...
    // global
    if (config["steps"]) n_iters = config["steps"].as<int>();
    ...
    if (config["mass"]) {
        if (cp.invMass == -1.0f) {
            cp.invMass = config["mass"].as<float>(cp.invMass);  
        }
    }
    ...
    if (config["bend_stiffness"] || config["stretch_stiffness"] || config["shear_stiffness"]) {
        ...
        float stiff_scale;
        if (g_clothStiffness <= 0.0) {  //<--- If the asssigned stiffness invalid.
...
void SDLInit(const char* title) {  //<--- [RENDER]
...
void InitSim() {            // <---- [DON'T CHANGE] Initialize the solver. This is the first step. 
	//cout<<"InitSim"<<endl;
	NvFlexInitDesc desc;
...
void ResetVisGlobals() {    // <----- [DON'T CHANGE] Define STATIC g_* parameters: don't update per-frame
...
void ResetSimGlobals() {   // <---- !!!!! Called only once. Initialize Relevent parameters and buffers. Run after Init(), before ResetVisGlobals()
...
void ResetSim() {     //<------ Called only once
	cout<<"ResetSim"<<endl;
	...
	// create scene
	StartGpuWork();
	g_scenes[g_scene]->Initialize();      //<---- !!!!! In the *.h file
	EndGpuWork();

	uint32_t numParticles = g_buffers->positions.size();    // <--- 44100
...
void Init(int scene, bool centerCamera)
{
	...
	ResetSimGlobals(); // maps buffers <---- Run only once
	ResetVisGlobals(); // <---- Run only once

	ResetCoupledAssets();
	ResetSim(); // unmaps buffers

	if (!g_renderOff) {   //<---- Didn't run
		MapBuffers(g_buffers);
...
void UpdateWind()
{
	...
	g_windTime += g_dt;
	//
	// [Xiaolvzi] original: const Vec3 kWindDir = Vec3(3.0f, 15.0f, 0.0f); 
	const Vec3 kWindDir = Vec3(3.0f, 0.0f, 15.0f);    // <---- Define wind direction here
	const float kNoise = Perlin1D(g_windTime*g_windFrequency, 10, 0.25f); 
	// [Xiaolvzi] Vec3 wind = g_windStrength*kWindDir*Vec3(kNoise, fabsf(kNoise), 0.0f);
...
void UpdateSimPostmap() {   //<----- [Xiaolvzi]: Frame update
	...
	g_shapesChanged = true; // <--- [Xiaolvzi] Added this.



	if (g_shapesChanged)    // <------ This runs at each frame
	{
		//cout <<"11111" << endl;
		//cout << g_buffers->positions[10] << endl; //<---- Stored in GPU: device pointer
		//cout << "2222" << endl;
		//cout << g_buffers->shapePositions.size() << endl;
		// [XiaoLvZi]: Create collision mesh

		//shapeRotation = shapePrevRotations+10;
		NvFlexSetShapes(  // <----- [XiaoLvZi]: This updates the geometry of the colliding object; But you can't change the parameters here: They have been sent to GPU. 
			g_solver,
			g_buffers->shapeGeometry.buffer,   //<---- FlexCollisionGeometry objects: triangle, sphere, capsule, plane... If triangle mesh, it has two variables: the scale of mesh and mesh itself.
			g_buffers->shapePositions.buffer,
			g_buffers->shapeRotations.buffer,  //<<---- Note that the method requires you to provide the transformation of the current frame and that one of the previous frame. This means that every frame, the current transformation gets stored in the ‘previous’ one and gets updated with a new one
			g_buffers->shapePrevPositions.buffer,
			g_buffers->shapePrevRotations.buffer,
			g_buffers->shapeFlags.buffer,
			int(g_buffers->shapeFlags.size()));

		//shapePrevRotations = shapeRotation;

		//g_shapesChanged = false; <--- [XiaoLvZi]: Commend this
	}
...
void PhotoShoot() { // <---- Isn't't useful
...
int main(int argc, char* argv[])
{
	...
	int cloth_size = Ball::AUTO_CLOTH_SIZE; // will autocompute
	int iters = 5000;
	float obj_scale = 2.0;   // scale of the underneath object + cloth
	...
	Ball::ClothParams cp(cloth_size, particle_radius, mass, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
	Ball::ObjParams op(obj_scale, translate, rotate, use_quat); //<-- INitialize the values

    if (parse) {
        ParseConfig(&cfg[0], iters, cp, op);
    }

	g_scenes.push_back(new Ball("Ball", &objpath[0], cp, op));
