...
void CreateParticleSphere(Vec3 center, int dim, float radius, Vec3 velocity, float invMass, bool rigid, float rigidStiffness, int phase, float jitter=0.000f)
{
	if (rigid && g_buffers->rigidIndices.empty())
			g_buffers->rigidOffsets.push_back(0);
...
void CreateParticleShape(const Mesh* srcMesh, Vec3 lower, Vec3 scale,
	...
	float rigidStiffness, int phase, bool skin, float jitter=0.005f, Vec3 skinOffset=0.0f,
...
NvFlexTriangleMeshId CreateTriangleMesh(Mesh* m, bool render=true)
{
	if (!m)
		return 0;
	...
	// cout << "      Add triangle mesh to Flex" << endl;
	...
	if (render) {
		// cout << "      Create GPU Mesh" << endl;
		g_meshes[flexMesh] = CreateGpuMesh(m);
	} else {
		g_meshes[flexMesh] = nullptr;
	}
...
void CreateSpringGrid(Vec3 lower, int dx, int dy, int dz, float radius, int phase, float stretchStiffness, float bendStiffness, float shearStiffness, Vec3 velocity, float invMass, float extra_spacing=0.0f, float extra_rad_mult=1.0f)
{
	...
	for (int z=0; z < dz; ++z)
	{
		for (int y=0; y < dy; ++y)
		{
			for (int x=0; x < dx; ++x)
			{
				Vec3 position = lower + (extra_rad_mult*radius + extra_spacing)*Vec3(float(x), float(z), float(y));
