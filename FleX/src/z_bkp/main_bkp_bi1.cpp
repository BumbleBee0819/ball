...
void ParseConfig(const char* path, int& n_iters, Occlusion::ClothParams& cp, Occlusion::ObjParams& op) {
...
void UpdateSimPostmap() {
	...
	NvFlexSetActiveCount(g_solver, g_buffers->activeIndices.sSize());
...
int main(int argc, char* argv[])
{
	...
	int cloth_size = Occlusion::AUTO_CLOTH_SIZE; // will autocompute
	int iters = 5000;
	float obj_scale = 2.0;
    ...
    */
	...
	Occlusion::ClothParams cp(cloth_size, particle_radius, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
	Occlusion::ObjParams op(obj_scale, translate, rotate, use_quat);

    if (parse) {
        ParseConfig(&cfg[0], iters, cp, op);
    }

	g_scenes.push_back(new Occlusion("Occlusion", &objpath[0], cp, op));
