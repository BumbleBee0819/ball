...
void PrintSimParams() {  	// [wb] ---- Print simulation params: g_params
	printf("\n ---- Simulation Params ----\n");
    printf("substeps:                  %d\n", g_numSubsteps);
    printf("radius:                    %f\n", g_params.radius);
    printf("num_planes:                %d\n", g_params.numPlanes);
...
void ParseConfig(const char* path, int& n_iters, Ball::ClothParams& cp, Ball::ObjParams& op) {
    ...
    if (config["bend_stiffness"] || config["stretch_stiffness"] || config["shear_stiffness"]) {   //<--- Set bs, ss, shear stiffness
    	if (config["stiffness"]) {
...
void SDLInit(const char* title)   //<--- [DON'T CHANGE]
{
...
int main(int argc, char* argv[])
{
	...
	float bend_stiffness = 2.0f;     // <---- Change bs here....
	...
	Ball::ClothParams cp(cloth_size, particle_radius, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
