...
public:
	...
	Bench(const char* name, const char* objpath, ClothParams cp, ObjParams op, float contact_eps=0.05f):
	...
	virtual void Initialize()
	{
		...
		obj_center = 0.5f*(obj_upper + obj_lower);
		...
		g_windStrength = 0.0f;   // <----- Change wind strength here.
		...
		Mesh* mesh_ball = ImportMesh(GetFilePathByPlatform("../../data/box.ply").c_str());
	...
	void Update() {
		//DetectTears(); // Amir's edit: disable doing unnecessary calculations		// // DetectTears(); // Amir's edit: disable doing unnecessary calculations
        //ClearShapes();

		mTime += g_dt;    // <---- 1/60 = 0.16667
		
		//float startTime = 1.0f;   // <----- let cloth settle on object
		...
		const float translationSpeed = 0.1f;   // <----- [Important!!!] This value has to be small, otherwise it will cause errors...
		const float initalHeight = 0.7f;
		// 2.0f is too large/
		//const float translationSpeed_z = 2.0f;

		Vec3 pos = Vec3(obj_center.x+0.3f, Max(0.3f, initalHeight-translationSpeed*time), obj_center.y);   //<--- This is in (x, z, y)
		Vec3 prevPos = Vec3(obj_center.x+0.3f, Max(0.3f, initalHeight-translationSpeed*lastTime), obj_center.y);

		// cout << pos.x << endl;
		// cout << pos.y << endl;
		// cout << pos.z << endl;
		
		//Vec3 pos = Vec3(obj_center.x, obj_center.z-translationSpeed_z*(1.0f-cosf(time)), translationSpeed*(1.0f-cosf(time)));   //<--- This is in (x, z, y)
		//Vec3 prevPos = Vec3(obj_center.x, obj_center.z-translationSpeed_z*(1.0f-cosf(lastTime)), translationSpeed*(1.0f-cosf(lastTime)));
