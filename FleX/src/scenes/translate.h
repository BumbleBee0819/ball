...
public:
	...
	virtual void Initialize()   // <---- Run only once
	{
		...
		// make cloth
		cloth_base_idx = int(g_buffers->positions.size());  // <------[XiaoLvZi]: 0
	...
	void Update() {
		...
		Vec3 pos = Vec3(translationSpeed*(1.0f-cosf(time)), obj_center.y, obj_center.z);
		Vec3 prevPos = Vec3(translationSpeed*(1.0f-cosf(lastTime)), obj_center.y, obj_center.z);
