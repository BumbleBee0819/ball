...
void ParseConfig(const char* path, int& n_iters, RotateTable::ClothParams& cp, RotateTable::ObjParams& op) {
...
void ResetSimGlobals() {   // <---- Called only once. Initialize. Run after Init(), before ResetVisGlobals()
...
int main(int argc, char* argv[])
{
    ...
    int cloth_size = RotateTable::AUTO_CLOTH_SIZE; // will autocompute
    int iters = 5000;
    float obj_scale = 2.0;
...
for (int i = 1; i < argc; ++i) {
    ...
    RotateTable::ClothParams cp(cloth_size, particle_radius, mass, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
    RotateTable::ObjParams op(obj_scale, translate, rotate, use_quat);
    ...
    // ------- [wbi: save params] ------- //
    std::string config_file_saved;
    std::string tmp(g_exportBase);
    std::ofstream outfile;

    config_file_saved = tmp + "config.txt";
    outfile.open(config_file_saved, std::ios_base::app);
    outfile << "\n ---- Cloth Params ----";
    outfile << "\n n_particles(per dim):   ";
    outfile << cp.n_particles;
    outfile << "\n particle_radius:   ";
    outfile << cp.particle_radius;
    outfile << "\n mass:   ";
    outfile << cp.invMass;
    outfile << "\n stretch_stiffness:   ";
    outfile << cp.stretch_stiffness;
    outfile << "\n bend_stiffness:   ";
    outfile << cp.bend_stiffness;
    outfile << "\n shear_stiffness:   ";
    outfile << cp.shear_stiffness;
    outfile << "\n extra_cp_spacing:   ";
    outfile << cp.extra_cp_spacing;
    outfile << "\n extra_cp_rad_mult:   ";
    outfile << cp.extra_cp_rad_mult;
    outfile << "\n -----------------------\n";
    outfile.close();
    // ------- [wbi: End save params] ------- //
    

    g_scenes.push_back(new RotateTable("RotateTable", &objpath[0], cp, op));
