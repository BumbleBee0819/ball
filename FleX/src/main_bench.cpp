...
void ParseConfig(const char* path, int& n_iters, Bench::ClothParams& cp, Bench::ObjParams& op) {
...
int main(int argc, char* argv[])
{
    ...
    int cloth_size = Bench::AUTO_CLOTH_SIZE; // will autocompute
    int iters = 5000;
    float obj_scale = 2.0;
...
for (int i = 1; i < argc; ++i) {
    ...
    Bench::ClothParams cp(cloth_size, particle_radius, mass, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
    Bench::ObjParams op(obj_scale, translate, rotate, use_quat);
    ...
    g_scenes.push_back(new Bench("Bench", &objpath[0], cp, op));
