...
void InitSim() {            // <---- [DON'T CHANGE] Initialize the solver. This is the first step (run once: step 1).
    //cout<<"InitSim"<<endl;
    NvFlexInitDesc desc;
...
void ResetVisGlobals() {    // <----- [RENDER] (run once: step 4)
...
void ResetSimGlobals() {   // <----- IMPORTANT!!!!  (run once: step 3)
    ...
    g_waveTime = 0.0f;
    g_windTime = 0.0f;
    g_windStrength = 0.5f;    // <--- will replace -windstrength
    g_windFrequency = 0.1*(1.0f/g_dt);  //wind by wbi
...
void OrientCamera(float angle=0.0f, float tilt=0.0f) {  //<--- // [RENDER]  Angle and tilt should be in degrees 
...
void ResetRender(bool centerCamera) { //<--- [RENDER: DON'T CHANGE]
...
void ResetSim() {       // <----- IMPORTANT!!!! (run once: step 6)
    //cout<<"ResetSim"<<endl;
...
void ResetCoupledAssets() {  // <----- [DON'T CHANGE] (run once: step 5)
...
void Init(int scene, bool centerCamera) {    // <----- [DON'T CHANGE](run once: step 2)
    //cout<<"Init"<<endl;
    RandInit();

    ResetSimGlobals(); // maps buffers
    ResetVisGlobals(); // don't change

    ResetCoupledAssets();  //don't change
...
void Reset() {     //<--- [DON'T CHANGE]
    //cout<<"Reset"<<endl;
    Init(g_scene, false);
}


void Shutdown() {      //<--- [DON'T CHANGE]
    //cout<<"Shutdown"<<endl;
    // free buffers
    DestroyBuffers(g_buffers);
...
void UpdateWind() {  // <----- (Loop: step 3)
    //cout << g_windStrength <<endl;
    g_windTime += g_dt;
    //x: left-right
    //y: vertical
    //z: front-back
    const Vec3 kWindDir = Vec3(10.0f, 3.0f, 9.0f);  //[wb] wind by wbi
    //const Vec3 kWindDir = Vec3(3.0f, 15.0f, 0.0f);
    float kNoise = Perlin1D(g_windTime*g_windFrequency, 10, 1.0f); //wind by wbi
    float x_wind = sinf(float(g_windTime)*g_waveFrequency*8.0f);
    if (x_wind < 0.6f) {
    	x_wind = abs(x_wind)/20.0f;
    	kNoise = abs(kNoise)/20.0f;
    }

    // Vec3 wind = g_windStrength*kWindDir*Vec3(kNoise, fabsf(kNoise), 0.0f);
    Vec3 wind = g_windStrength*kWindDir*Vec3(x_wind, 0.0f, kNoise);
    // cout<<wind.x<<endl;
    // cout<<abs(x_wind)<<endl;
    // cout<<wind.z<<endl;
    // cout<<'\n'<<endl;
...
void UpdateSimMapped() {    //<--- (Loop: step 2)
...
void UpdateSimPostmap() {    //<--- IMPORTANT!!! (Loop: step 5)
...
void UpdateRenderPostmap() {     //<--- [RENDER]
...
void UpdateFrame(bool render=true) {      //<--- (Loop: step 1)
...
void MainLoop(int n_iters) { //<----- (run once: step 7, last)
    //cout<<"MainLoop"<<endl;
...
void setCapture() {           //<--- [DON'T CHANGE]
    g_showHelp = false;
    g_takeSnaps = true;
}


bool exists(const char* p, bool verbose=false) {         //<--- [DON'T CHANGE]
    FILE *f = fopen(p, "r");
    if (f) {
        fclose(f);
        return true;
...
int main(int argc, char* argv[]) {
    ...
    Wind::ClothParams cp(cloth_size, particle_radius, mass, stretch_stiffness, bend_stiffness, shear_stiffness, extra_cp_spacing, extra_cp_rad_mult);
    Wind::ObjParams op(obj_scale, translate, rotate, use_quat);


    if (parse) {   //<----- Add params from the config file.
